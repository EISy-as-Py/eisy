...
def test_to_dataframe():
 ...
 the high frequency limit value. Check again.'
    np.testing.assert_almost_equal(len(f_range[1]), impedance_arr[0].shape[0],
                                   decimal=18, err_msg='the impedance\
 is not correclty computed. The number of points in the array is not correct.')
    assert len(f_range[1]) == impedance_arr[0].shape[0], 'the frequency and\
 ...
 the impedance response should be populated by complex numberes'
    assert dataframe.columns[0] == 'freq [Hz]', 'the first column\
 should contain the frequency respose.'
    assert dataframe.columns[1] == 'angular_freq [1/s]', 'the second column\
 should contain the angular frequency respose.'
    assert dataframe.columns[2] == 'Re_Z [ohm]', 'the third column\
 should contain the real impedance part.'
 ...
 should contain the frequency respose. Check again'
    assert dataframe['angular_freq [1/s]'][0] == f_range[1][0], 'the second\
 column should contain the angular frequency respose. Check again'
 ...
 column should contain the real impedance respose. Check again'
    ...
    assert isinstance(dataframe['freq [Hz]'][0], float), 'the dataframe\
 contains the wrok data type'
...
an integer'
    assert high_freq >= low_freq, 'the low frequency should be smaller than\
 the high frequency limit value. Check again.'
    np.testing.assert_almost_equal(len(f_range[1]), impedance[0].shape[0],
                                   decimal=18, err_msg='the impedance\
is not correclty computed. The number of points in the array is not correct.')
    assert len(f_range[1]) == impedance[0].shape[0], 'the frequency and\
 the impedance respons do not match in length. Check again.'
    assert len(impedance) == 5, 'the impedance array inputted is not\
 the right dimensions. The number of columns exceed the expected value (5)'
    assert isinstance(impedance[0][1], complex), 'the first column of\
 the impedance response should be populated by complex numberes'
    assert circuit[0].real == impedance[1][0], 'the complex impedance is not\
 separated into its real and imaginary parts correctly.'
    assert circuit[0].imag == impedance[2][0], 'the complex impedance is not\
 separated into its real and imaginary parts correctly.'
    assert isinstance(impedance[1][0], float), 'the impedance array contains\
 the wrong type of data'
    assert isinstance(impedance[2][0], float), 'the impedance array contains\
the wrong type of data'


def test_RC_simuation():
...
is not correclty computed. The number of points in the array is not correct.')
    ...
    assert isinstance(impedance_data_s_df, pd.DataFrame), \
        'The output should be a pandas.DataFrame'
    assert len(f_range[1]) == impedance_data_p[0].shape[0], 'the frequency and\
 the impedance respons do not match in length. Check again.'
    assert isinstance(circuit_configuration_p, str), 'the circuit\
 configuration should be a string.'
    assert len(impedance_data_p) == 5, 'the impedance array inputted is not\
 the right dimensions. The number of columns exceed the expected value (5)'
    assert isinstance(impedance_data_s[0][1], complex), 'the first column of\
 the impedance response should be populated by complex numberes'
    assert circuit_parallel[0].real == impedance_data_p[1][0], 'the complex\
 impedance is not separated into its real and imaginary parts correctly.'
    assert circuit_parallel[0].imag == impedance_data_p[2][0], 'the complex\
 impedance is not separated into its real and imaginary parts correctly.'
    assert isinstance(circuit_configuration_s, str), 'the circuit\
 configuration should be a string.'
    assert len(f_range[1]) == impedance_data_s[0].shape[0], 'the frequency and\
the impedance respons do not match in length. Check again.'
    assert len(impedance_data_s) == 5, 'the impedance array inputted is not\
the right dimensions. The number of columns exceed the expected value (5)'
    assert isinstance(impedance_data_s[0][1], complex), 'the first column of\
the impedance response should be populated by complex numberes'
